...
function Header({ locale, current }: { locale: Locale; current: string }) {
  ...
  return <header className="site-header"><a className="brand" href={toPath(locale)} aria-label="Sihan Wang home">Sihan <span>Wang</span></a><nav aria-label="Primary navigation"><a className={current.startsWith("work") ? "active" : ""} href={toPath(locale, "work")}>{t.nav.work}</a><a className={current === "experience" ? "active" : ""} href={toPath(locale, "experience")}>{t.nav.experience}</a></nav><div className="header-actions"><a className="locale-toggle" href={toPath(toggle, suffix.replace(/^\//, ""))}>{toggle.toUpperCase()}</a><a className="contact-dot" href="mailto:sihan006@e.ntu.edu.sg">{t.nav.contact}</a></div></header>;
}
